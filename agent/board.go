...
func NewBoard(g GameSnapshot) *Board {
    ...
    for _, food := range g.Food() {
        if food.Y < g.Height() && food.X < g.Width() {
            board.Cells[food.Y][food.X] = FoodCell{}
        }
    }
    ...
    for _, snake := range g.Snakes() {
        ...
        if body[0].Y < g.Height() && body[0].X < g.Width() {
            board.Cells[body[0].Y][body[0].X] = SnakePartCell{
                SnakeID:  snake.ID(),
        ...
        for i := 1; i < len(body)-1; i++ {
            if body[i].Y < g.Height() && body[i].X < g.Width() {
                board.Cells[body[i].Y][body[i].X] = SnakePartCell{
                    SnakeID:  snake.ID(),
        ...
        if len(body) > 1 {
            ...
            if tail.Y < g.Height() && tail.X < g.Width() {
                board.Cells[tail.Y][tail.X] = SnakePartCell{
                    SnakeID:            snake.ID(),
